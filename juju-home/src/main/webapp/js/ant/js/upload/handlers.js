...
function initSwfupload(url,sizeLimit,types,typsdesc,uploadLimit){
		...
		var settings={
			...
			flash_url : "/ant/js/upload/swfupload.swf",
			file_post_name:"filedata",
			
			upload_url: url,// "/servlet/FileUploadServlet.htm?jsessionId=${pageContext.session.id}",
			...
			// Button Settings
			button_image_url : "/ant/images/select-files-zh_CN.png",
...
function insertAllevent(){
	...
	$("img[data-status='0']").each(function(i,obj){
			...
			$("#showImages").empty();
			var img = "<img src="+url+" style='width:90px;height:90px;'/><input id='thumbnail' value="+url+" type='hidden' name='thumbnail'/>";
...
function addReadyFileInfo(fileid,fileName,message){
	var photoDiv='<div class="ke-inline-block ke-item" id="'+fileid+'">'
			...
			+'	<img src="/ant/images/image.png" class="ke-img" data-status="-1" width="80" height="80" />'
